...
# Execute it like "upgrade.sh [namespace] [deployment] [image]
...
echo "Upgrading deployment $2 in namespace $1 with image $3 ..."

rancher kubectl \
    --namespace=$1 \
    patch deployment $2 \
    --type=strategic \
    -p '{"spec":{"template":{"spec":{"containers":[{"name":"'$2'","image":"'$3'","env":[{"name":"FORCE_RESTART_AT","value":"'$(date --utc -Iseconds)'"}]}]}}}}'
